...
index.html 
  ...
  - 置換コマンドで、g は x軸、% は y 軸を対象とする。
  - c オプションをつけると、ひとつずつ確認できる。
  - g& で置換コマンドに % をつけたものをファイル全体に対して適用することができる。
  - 次の二つのコマンド(の拡張)でcsvの列を入れ替えられる
    - /\v^([^,]*),([^,]*),([^,]*)$
    - :%s/\3,\2,\1
  - 次のコマンドで複数ファイルにわたって文字を置換できる
    1. vi
    2. /word
    3. :args *
    4. :set hidden
  ...
  - grepprg 設定は :grep コマンド実行時に、シェルで何をするべきかを指定する。
  - grepformat 設定は :grep コマンドからの出力する方法を vim に伝える。
...
# Blender Python
  ...
  - タプルの要素の値の変更は不可。
  - bpy.ops.mesh.primitive_cube_add( の後に Tab を押すと引数一覧とデフォルト値が出てくる。
  - bpy.data.materials には blender 上で作成したすべてのマテリアルが保存されている。
  - 関数内で global と宣言すると global 変数にすることができる。
    - global 変数の参照の際は global は必要ない。
  - bpy.data.materials はすべてのマテリアル。
  - bpy.data.materials.new で新たなマテリアルを作成する。
  - bpy.ops.object.material_slot_add でアクティブなオブジェクトにマテリアルスロットを追加する。
  - クラスとは、あるオブジェクトの属性と操作をまとめた設計図
    - オブジェクトの属性はメンバ変数、操作はメンバ関数と呼ばれる。
    - オブジェクトとして具現化することをインスタンス化という。
    - メリット
      - カプセル化: クラス利用者が利用できるメンバ変数やメンバ関数を限定し、内部の実装を抽象化することでプログラムの可読性や保守性が向上する。
      - 継承: クラス間に親子関係を持たせ、メンバ変数やメンバ関数を一部共有することで、重複したソースコードを記述する必要がなくなる。
      - ポリモフィズム: 同一名で動作が異なるメンバ関数を親子間で定義した場合、実行時に適切なメンバ関数を選んで実行するため、ソースコードの記述が簡潔になる。
    - メンバ関数のメンバ変数へは、self を通じてアクセスする。
    - インスタンス化時に処理される関数をコンストラクタという。
      - ロボットのモデルはインスタンス化した時点で作成される方が自然など。
    - オブジェクト削除時に実行する処理はデストラクタで行う。
  - Python では次の型の値を変数や引数へ代入するときに値渡しとなる。
    - 整数型
    - 浮動小数点型
    - 文字列型
    - ブーリアン
      - これ以外では参照渡しとなる。
  - copy モジュールの copy 関数は1段階層までのコピーしかされない(リストのリストなど、中のリストは参照渡しとなる)
    - deepcopy 関数で２段階以上の深さまで値のコピーがされる。
  - super().__init__() で親クラスのコンストラクタを呼び出す。
  - クラスや関数、変数が定義されたファイルのことをモジュールと呼ぶ。
    - あるモジュールに定義されたクラスを利用する場合、<モジュール名>.<クラス名>と記述する。
  - Python プログラムは検索対象のパスからインポートするモジュールを検索する。
    - インポートしたいモジュールが検索対象のパスに含まれていない場合、モジュールが見つからないと判断してエラーを発生させる。
    - sys.path に検索パスが保存されている。
  - r 文字列 (r" ") によりエスケープシーケンスを無効化することができる。
  - 複数のモジュールを一つにまとめたものをパッケージという。
    - パッケージの作成手順
      - パッケージと同名のディレクトリ名を作成する。
      - 作成したディレクトリにファイル \_\_init\_\_.py と、パッケージに含める全モジュールのファイルを配置する。
  - from 文を用いると現在のパッケージからの相対パスで指定したパッケージに所属するモジュールをインポートできる。
  - __init__.py に追加した処理は、パッケージをインポートしたときに実行される。
  - blender python ではパッケージもモジュールと呼ばれる。
  - context モジュール
    - Blender の実行状況 (コンテキスト) へのアクセス手段を提供。
    - コンテキストとは、実行時の状態によって異なる振る舞いをすることを表した用語。
    - 実行状態に応じてアクセス可能なデータが変わる。
      - アクティブな状態など。
  - data モジュールは Blender の内部データへのアクセス手段を提供
  ...
  - アドオンの提供形態は次の３つに分類される。
    - Release
      - リリース版のBlender本体に同梱される。
    - Contrib
      - リリース版のBlender本体には同梱されないが、テストビルドされたBlender本体に同梱される。
        - テストビルドされたBlenderとは、最新のソースコードを使用してビルドされたBlender。安定していないことが多い。
    - External
      - Blender本体に同梱されず、アドオン開発者個人もしくは外部団体によって提供される。
      - 一般的なアドオンの提供形態。
  - アドオン向けの Pytho プログラムでは、連想配列の変数 bl_info を定義する必要がある。
    - 次の情報が必要となる
      - name: アドオンの名前
        - 機能を表すように
      - author: アドオンの作者
        - 複数いる場合はカンマ区切りに書く
      - version: アドオンのバージョン
        - メジャーバージョン(互換性のない大幅な変更)、マイナーバージョン(機能追加)、バッチバージョン(バグ修正)
      - blender: アドオンが動作する Blender 本体の最古バージョン
        - これよりも古いバージョンを指定した場合でも警告が出るだけで使用することはできる。
      - location: アドオンが提供する機能が存在する場所
      - description: アドオンの説明
      - warning: アドオン使用時の注意
      - support: アドオンのサポートレベル
        - OFFICIAL(Blender開発者のみ)、COMUNITY(個人開発者)、TESTING(テスト中)
      - doc_url: アドオンに関する情報が得られるサイトの URL
      - tracker_url: アドオンに関するサポートサイトの URL
        - 開発者にバグ報告などができる URL
      - category: アドオンのカテゴリー
        - できるだけ既存の URL を使用する。
  - アドオン向けの Python プログラムには register関数と unregister 関数を定義する必要がある。
    - register関数はアドオン有効化時に実行される。
    - unregister関数はアドオン無効化時に実行される。
  - オペレータクラスのクラス名は XXX_OT_YYY の形式に従う必要がある。
  - メンバ変数にはクラス変数と、インスタンス変数がある。
    - クラス変数は、インスタンス間で値を共有する
    - インスタンス変数はインスタンス間で値を共有せず、インスタンスごとに値をもつ。
  - クラス変数 bl_idname はオペレータを識別する文字列を指定し、この文字列は Blender 内で唯一となる必要がある。 
  - オペレータクラスを定義するだけでは blender 上で実行できないので、呼び出し用 API で呼び出す(bpy.utils.register_class)
  - Python ではクラスを定義した後でもクラスにメンバ変数を追加できる。
  - メニューにカスタムオペレータを追加する場合は、bpy.types.Menu を継承する。
  - サイドバーにアドオンの機能を追加する場合は、bpy.types.Panel を継承する。
  - ツールバーにアドオンの機能を追加する場合は、bpy.types.WorkSpaceTool を継承する。
    - ワークスペースツールのクラス名には、使うべき形式は存在しない。
  - アドオンプリファレンスクラスで定義したクラス変数は、クラス bpy.types.Context のメンバ変数 preference.user_prefs.addons[__name__].preferencesに保存されている。
  - マウスやキーボードからの入力を受け取り、その入力に基づいて処理を行うためにはイベントを扱う。
  - Blender で扱えるイベントは次の3つ。
    - マウス
      - ドラッグやクリックなどを行うとイベントが発生し、マウスのボタンの状態やマウスの位置を取得できる。
    - キーボード
      - キーを押したり話したりするとイベントが発生し、変更されたキーの状態を取得できる。
    - タイマ
      - 指定した間隔でイベントを発生させるタイマは、一定の間隔で処理を実行する場合に役立つ。
        - 例: マウスオーバーして一定時間後にツールチップを表示する。
  - イベントを利用したオペレータをモーダルオペレータという。
    - モーダルオペレータを実行すると、イベントを撮り続ける状態(モーダル)になり、その状態を解除するまで永続的にイベントを受け取り続ける。
  - for ～ else 文
    - for ループが正常に終了した後に else が実行される。
    - for のなかに break 文を入れた場合に正常に for ループが終わったか否かを判断できる。
  ...
  - ソースコードの代表的なライセンス
    - GNU General Public Licence (GPL)
      - Free Software Foundation によって公開されているライセンス。
      - このライセンスが適用されたソースコードによって作成されたプログラムは、利用者から要求があるときにソースコードを公開する義務がある。
      - GPL が適用されたソースコードは、改変や再配布か可能である。
        - GPL が適用されたプログラムやそのソースコードを利用した派生物についても、GPL の適用が必要となる。
        - GPL の適用範囲が派生物に及ぶため、「GPL 汚染」と呼ばれることがある。
    - MIT Licence
      - マサチューセッツ工科大学によって公開されているライセンス。
      - GPL と比較して制約が緩いライセンスで、X11 Licence とも呼ばれる。
      - MIT Licence が適用されたソースコードは、改変や再配布の時に著作権の表示と MIT Licence 本文の記載、無保証の表示のみが求められる。
      - 非公開にできるほか、適用するライセンスを変更して再配布が可能。
    - Apache Licence
      - Apache Software Foundation から公開されているライセンス。
      - MIT Licence よりも制約が緩いライセンスで、ライセンス本文の記載が不要。
      - 本ライセンスが適用されたソースコードを改変や再配布する場合は、著作権の表示とApache Licence が適用されていることを示す文章をソースコードに追加するだけで十分。
      - 再配布時に適用するライセンスを変更でき、ソースコードを非公開にもできる。
    - Berkeley Software Distribution Licence
      - カルフォルニア大学から公開されているライセンス。
      - MIT Licence とほぼ同様の制約を持つライセンスで、略して BSD ライセンスと呼ばれる。
        - いくつか種類があるが、ここでは修正 BSD ライセンスを指している。
      - 修正 BSD ライセンスが適用されたソースコードは、改変や再配布の時に著作権の表示と無保証の表示のみを要求する。
      - オリジナル版の BSD ライセンスは、GPL と両立できない。
  - Blender のライセンスは、基本的に GPLv2 以降のライセンスを適用したソースコードが公開されている。
    - Blender を構成するすべてのコンポーネントは GPL 互換という条件を満たす必要がある。
    - ライセンスが明記されていないものを利用するのはライセンス違反である。
  - VSCode 向けの拡張機能 Blender Development は Blender の Python プログラム開発を効率化する機能を VSCode に追加する。
    - テンプレートからソースコード生成
    - Python プログラムのデバッグ実行
    - アドオンの再読み込み
...
# Particle System
  ...
  - Emission モジュール
    ...
    - Bursts
      ...
      - Cycle が 1 より大きい場合は、さらに Interval の時間が経過した後にもう一度パーティクルが発生する。
      - Probability が 1 以下なら、確率で発生しなくなる。
  - Limit Velocity over Lifetime
    - パーティクルの速度に制限をかけることができる。
      - Speed で設定した値を超えると減速が発生する。
      - Dampen で大きな値を設定するほど、大きく減速するようになる。
      - Drag の値を設定すると、現在の速度に依らずパーティクルを減速させられる。
  - Rotation over Lifetime
    - 時間に伴う回転速度を設定する。
  - 自然に消滅させるように見せるには Color over Lifetime で alpha 値を調整した方がいいかもしれない。
  - Noise モジュールの Frequency をあげると、パーティクルの位置や回転が頻繁に変わるようになる。
  - Scroll Speed を大きくすると、ノイズマップ自体が時間経過で変わるようになる。
  - Trail モジュール
    - Die with Particle にチェックが入っている場合は、パーティクルが消滅した後に、トレイルも消滅する。
    - トレイルが存在出来る時間は、Main モジュールの Start Lifetime × Trail モジュールの Lifetime 
  - F キーを押すと、画面がオブジェクトの位置に移動する。
  - SubEmitter モジュール
    - 特定のパーティクルの発動条件を変えたり、親パーティクルの位置から子パーティクルが出現したりするように挙動を調整することができる。
    - 描画負荷の増刊繋がるため、注意。
...
## ゼロから始める UdonSharp
  - UdonSharp は、VRChat SDK 上で動作する C#。 
    - 基本的な書き方は C# と同じ。
  - C# スクリプトの構造
    ```C#
    // ディレクティブ
    ...
    using VRC.Udon;
    // クラス      クラス名 
    // スクリプトの中身。ここに関数や変数を書いて様々な処理を実装する。
    public class DebugButton : UdonSharpBehaviour
    {
        // 関数
        // このスクリプトが実行する処理を記載したもの。
        void Start()
        {
    ...
    ```  
      - クラス名は、作成した .cs ファイルの名前と一致する必要がある。
  - Start 関数
    - オブジェクトが最初にアクティブになった瞬間に 1回だけ実行する。
  - 関数を定義するときは、「返り値」「関数名」「引数」を書く。
  - デバッグを行うコツとして、Console やテキストにデータを出力する、効果音を鳴らす、が効果的。
  - Transform コンポーネントは、ゲームオブジェクトに必ず存在するコンポーネント。
  ...
  - SendCustomNetworkEvent()
    ...
    - 「引数がない」関数のみを発動できる。
  - [UdonSynced]をつけると変数の値を同期することができる。
  - SetOwner
    - 一つ目の引数でプレイヤーを指定し、2つ目の引数で指定したゲームオブジェクトの Owner にすることができる。
    - 1つのゲームオブジェクトの Owner になれるのは同時に１人のみ。
    - スクリプトで設定していない場合は、最初にワールドの読み込みが完了した人が全ゲームオブジェクトのOwner になる。
    - Owner がインスタンスを抜けた場合は、元の Owner の次に読み込みが完了した人が、新しい Owner になる。
  - GetOwner は引数でゲームオブジェクトを指定し、そのオブジェクトのオーナーになっているプレイヤーのデータが、返り値として戻ってくる。
  - RequestSerialization() を実行すると、データの同期が始まる。
    - 各プレイヤーの下にデータが到着すると、Owner 以外のプレイヤーは OnDeserialization() を自動的に実行する。
  - スクリプトの同期モード
    - Manual モード
      - RequestSerialization() を発動しない限り、データ同期は行われない。
      - SendWCustomNetworkEvent() を発動するとイベント同期が行われる。
      - プレイヤー間の無駄なデータ通信を減らすことができるので、安定してギミックが動作する。
      - 基本はこれを推奨。
    - Continuous モード
      - そこそこのペースで常にデータ同期が行われる。(RequestSerialization や OnDesirialization をスクリプトに書く必要はない。)
      - SendCustomNetworkEvent() を発動するとイベント同期が行われる。
      - VRC_ObjectSync をつけたオブジェクトはこれを選択するとよい。
      - データの同期の頻度は状況により変わり、オブジェクトや同期変数が多いと頻度が下がる。
      - データが自動補完され、Owner が 1 から 10 まで値を書き換えた場合、1->1.5->2.5->...->10 のようにある程度滑らかに変化させる処理が入る。
    - None モード
      - タイミング同期もデータ同期も一切行われない。
      - このモードは基本的に選択しなくても良い。
  - 同期の安定性 
    - 同期変数が多くなるとギミックの挙動が不安定になることがある。次のポイントに従うと、同期負荷を下げることができる。 
      |ポイント|理由|
      |--|--|
      |Continuous モードの UdonBehaviour は極力使わない。|常に通信帯域を占有するため。Continuous モードは、VRC_ObjectSync がついているオブジェクトだけに使うのがおすすめ。|
      |頻繁に Owner が変わったり、同期変数値が変わるオブジェクトは、同期変数を少なくする。|1つのオブジェクトに同期変数が複数ある場合、特定の同期変数だけを更新することはできないため。送受信するデータの容量を小さくすれば動作が安定する。|
      |なるべくデータ量が小さい変数型を使って同期を行う。|データ量が小さいほど、安定した同期ができるため。|
    - Continuous モードのオブジェクトは常にデータ通信を行うため、大量におくと同期が不安定になりがち。
    - 高性能な CPU、GPU を使っている場合でも、帯域はボトルネックになりがち。
    - Manual 同期の速度悪化にもつながるため、できるだけ、COntinuous モードは避け、不要な通信は押さえる。
    - あまり変わらない変数は別スクリプトで同期するなどが考えられる。
    - 変数はその型により、メモリ上で必要な領域が違う。例えば次の表のようになる。
      |変数型|ビット型|
      |--|--|
      |Int|32|
      |Float|32|
      ...
      |Vector3|96|
      |Quaternion|128|
        - 同期を行う際には、なるべくビット数が少ないものを選ぶ。
  - ミニゲームの作成
    - ギミックの構成を考える
      - 次のポイントを意識してギミックの動作を書き下すとよい
        - どのオブジェクトにスクリプトをつける必要があるか。
        - どんなコンポーネントが必要か。
        - 誰がスタート/ゴール時の処理を実施するか。
        - 発動するエフェクトはローカル/グローバルのどちらか。
    - OnPlayerTriggerEnter()
      - 誰かがトリガーコライダーに侵入したことを検知する。
    - ゲームオブジェクトが初期状態で無効化されていると、有効化されるまで同期が実行されない仕様がある。同期に関するゲームオブジェクトは、初期状態で無効化しないことが推奨される。
  - VRC の APi や Unity のコンポーネントを組み合わせて、所望の動作ができるオブジェクトを作るというのが、ギミック作成の基本。

  # 構造力学
  ## 構造力学って何ですか？
    - 板を川に渡して、その上を人がわたることを考える。このとき板は一つの構造物であり、渡る人の重さのことを外力、または荷重という。
    - 構造物とは荷重を支えることを意図した物体の集合ということができる。
    - 機能を支障なく果たすように構造物の寸法や形を決めることを構造物を設計するという。
    - 自ら移動しない荷重を死荷重、自ら移動するものを活荷重という。
    - 点に作用していると考えてよい荷重を集中荷重と言い、一つの矢印で抽象化して表す。
    - ある領域で分布している荷重は分布荷重という。
    - 構造物を構成する要素を部材という。
