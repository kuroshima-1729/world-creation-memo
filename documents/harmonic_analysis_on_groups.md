...
ルベーグ積分のアイディア
 - リーマン積分は、 $x$ 軸の分割を与えてから面積を近似していく。
 - ルベーグ積分は、 $y$ 軸の分割を与えてから、グラフの面積を近似していく。
 - 関数 $f: [a,b]\rightarrow \mathbb{R}$ が $f(x)\in [m, M] \  (m<M>)$ を満たすとき、 $m=y_0 < y_1 < \dots < y_N = M$ となるような区間 $[m,M]$ の分割を考え、各 $k=1,\dots, N$ に対し集合 $A_k\subset [a,b]$ を
   $$
   A_k := \{x\in [a,b]\mid f(x)\in [y_{k-1}, y_k)\}
   $$
   と定義する。(例外的に $A_N$ には $f(x)=y_N=M$ となる $x$ たちも加えておく)。その集合としての"長さ"を $\mu(A_k)$ と表す。 
 - $N$ が十分に大きく、分割の幅が小さければ、仮に「ルベーグ和」
   $$
   \displaystyle\sum_{k=1}^N y_k\cdot \mu (A_k)
   $$
   は積分の近似値を与えると考えられる。
...
可測集合とルベーグ測度</br>
命題 2.1 (2) より、任意の $A_1, A_2\in 2^{\mathbb{R}}$ に対し、
$$
\mu^*(A_1\cup A_2)\le \mu^*(A_1) + \mu^*(A_2)
$$
が成り立つ。そこで次の問題を考える。

問題</br>
$A_1\cap A_2=\emptyset$ のとき
$$
\mu^*(A_1\cup A_2) = \mu^*(A_1) + \mu^*(A_2)
$$
が成り立つか。

この問題は成り立たない。(ヴィタリ集合が反例となるらしい)。

可測集合</br>
今、任意に $A,B\in 2^{\mathbb{R}}$ が与えられたとき、 $A_1:=B\cap A, A_2:=B\cap A^c$ とおけば、
$$
\mu^*(B)\le \mu^*(B\cap A) + \mu^*(B\cap A^c)
$$
これを念頭に置いて、集合の可測性を次のように定義する。
